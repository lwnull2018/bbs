...
$(function() {
    ...
    layui.use(['form','laydate', 'upload'], function(){
        ...
        //绑定下拉框改变事件
        /*form.on('select(contentTypeSelect-1)', function (data){
            if (data.value==='Text') {
                $("#fileDiv-1").addClass("layui-hide");
                $("#textContentDiv-1").removeClass("layui-hide");
            } else {
                $("#fileDiv-1").removeClass("layui-hide");
                $("#textContentDiv-1").addClass("layui-hide");
            }
        ...
        upload.render({
            elem: '#fileContent'
            ,url: '/msgManage/uploadMultiFile' //此处配置你自己的上传接口即可
            ,auto: false
            ,accept: 'file'
            //,multiple: true
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.code == 1) {
                ...
                // console.log("fileList size = " + attachmentFileList.size());
